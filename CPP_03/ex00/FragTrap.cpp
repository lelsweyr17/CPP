...
FragTrap::~FragTrap()
{
	...
	std::cout << name << ITALIC BOLD << " : \"" << quotes[rand() % 3] << "\"" << RESET RESET << std::endl << std::endl;
...
{
	std::cout << ITALIC << "\t\t\t\t\t\t* CREATE CORY FRAGTRAP *" << RESET << std::endl << std::endl;
	std::cout << name << ITALIC << " : \"" << "Are you ready to fight with clone?" << "\"" << RESET << std::endl << std::endl;
}
...
void	FragTrap::randomActionQuotes(std::string enemyName)
{
	...
	std::cout << YELLOW ITALIC BOLD << enemyName << " : \"" << quotes[rand() % 9] << "\"" << RESET RESET RESET << std::endl << std::endl;
